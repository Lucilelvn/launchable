  ...
  return {
    ...
    competition: {
      ...
      evidence: [
        ...
        { text: 'No direct competitor offers AI-generated build prompts as an output', source: '' },
        { text: 'Most existing tools require extensive input — surveys, interviews, landing pages', source: '' },
      ],
    },
    shippability: {
      score: 9,
...
export async function mockRefine(): Promise<Refinement> {
  ...
  return {
    ...
    features: [
      { id: 'f1', name: 'One-sentence idea input', description: 'Simple form where users describe their idea in plain English — no jargon required', priority: 'must-have', complexity: 'low', user_appetite: 'First-timers are overwhelmed by lengthy forms. A single sentence removes friction and gets them started.', accepted: true },
      { id: 'f2', name: 'Web-researched scoring', description: 'AI searches the web for demand signals and competitors before scoring the idea', priority: 'must-have', complexity: 'medium', user_appetite: 'Manual competitor research takes hours and Alex usually gives up halfway through.', accepted: true },
      { id: 'f3', name: 'Evidence-backed dimension cards', description: 'Show demand, competition, and shippability scores with clickable evidence from web research', priority: 'must-have', complexity: 'medium', user_appetite: 'Alex needs to see WHY an idea scored the way it did, not just a number.', accepted: true },
      { id: 'f4', name: 'Idea mutations', description: 'Generate 3 alternative directions: pivot, niche down, or expand the idea', priority: 'must-have', complexity: 'low', user_appetite: 'Alex\'s best ideas often come from tweaking an existing concept, not starting from scratch.', accepted: true },
      { id: 'f5', name: 'Persona generation', description: 'AI generates a target user persona with pain points based on the idea and assessment', priority: 'nice-to-have', complexity: 'low', user_appetite: 'Helps Alex think about who they\'re building for instead of building for everyone.', accepted: true },
      { id: 'f6', name: 'Feature curator', description: 'Interactive checklist of AI-suggested features that can be accepted, rejected, or customized', priority: 'nice-to-have', complexity: 'medium', user_appetite: 'Alex always over-scopes. A curated list with complexity ratings prevents feature creep.', accepted: true },
      { id: 'f7', name: 'Ready-to-paste build prompt', description: 'Generate a detailed prompt tailored to the selected features and chosen AI coding tool', priority: 'must-have', complexity: 'medium', user_appetite: 'The whole point — Alex wants to paste something into Claude Code and start building immediately.', accepted: true },
    ],
  };
}
...
export async function mockGenerateTitle(concept: string): Promise<string> {
  ...
  // Generate a simple title from the concept
  const words = concept.split(/\s+/);
  if (words.length <= 5) return concept;
  // Take key nouns/verbs, skip articles and prepositions
